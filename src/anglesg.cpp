...
std::tuple<Matrix&, Matrix&> anglesg(double az1, double az2, double az3, double el1, double el2, double el3, double Mjd1, double Mjd2, double Mjd3, Matrix Rs1, Matrix Rs2, Matrix Rs3) {
    ...
    poly(4)=  0.0;
    poly(5)=  0.0;
    poly(6)=  -Constants::GM_Earth*(d2s*Ccye + 2*d1s*d2s);
    poly(7)=  0.0;
    poly(8)=  0.0;
    poly(9)=  -Constants::GM_Earth^2*d2s^2;
    rootarr = roots( poly );
    ...
    while ((abs(rhoold2-rho2) > 1e-12) && (ll <= 0 ))
        ...
        if ( ll <= 8  {
            u = Constants::GM_Earth/magr2^3;
            rdot= dot(r2,v2)/magr2;
            udot= (-3*Constants::GM_Earth*rdot)/(magr2^4);
